...
export default function ({authUser}) {
    ...
    useEffect(() => {
      ...
      useEffect(()=> {
          if(cardArray.decks){
          cardArray.decks.forEach(card => setDeckArray(deckArray => [...deckArray, card]))
               }
      }, [cardArray.decks])
        console.log("YUP")
    ...
    const confirmDeck = async(e) => {
      ...
      const response = await  fetch("http://localhost:3000/card/deck", requestOptions);
        ...
        {showInfo==="true" ? <InfoBox card={selectedCard}/>:<DeckBox />} 
          ...
          <div className="row">
              <div style={{minWidth:250}}>
               ...
               <h3 className="build">BUILD DECK {deckArray.length}  &#160;&#160;&#160;&#160;  <button className="deckOK" onClick={confirmDeck}><b>OK</b></button></h3>{deckArray.map((card, key) => 
               <div className="panelBox" onClick={() => handlePanelClick(card)}>{card.nation} God {card.name} <button className="deckX" onClick={()=>{removeFromDeck(card)}} >X</button>
               </div>
               )}</div>
